...
BEGIN
  ...
  LOOP
    IF char_array[ind]::int % 2 = 0 THEN
      -- when an even digit is found, pfcode basin upstream limit (codeup) is the basin code padded to the right with 9s
      -- the downstream limit (codedown) is pfcode itself - return with the limits
      limits.codedown := pfcode;
      limits.codeup := rpad(array_to_string(char_array,''),35,'9');
      RETURN limits;
    END IF;
    ind := ind - 1;
